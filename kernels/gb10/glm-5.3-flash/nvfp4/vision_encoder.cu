...
// Atlas Spark — GLM-5.3-Flash Vision Encoder CUDA Kernels
//
// All ops use BF16 storage for weights; computations use f32 accumulators.
//
// Everything above the GLM section is a VERBATIM copy of
// kernels/gb10/qwen3.8-flash-next/nvfp4/vision_encoder.cu. The copy is
// deliberate and total, not a cherry-pick: the encoder init table shared by
// both towers hard-requires vision_gemm_bias, vision_layer_norm,
// vision_add_inplace, vision_gelu, vision_attention_rope,
// vision_spatial_merge, vision_f32_to_bf16 and vision_bf16_copy, and soft-
// resolves the GEMM-ViT quintet. Dropping any hard-required symbol — even one
// GLM's forward path never calls, which is true of both vision_gelu and
// vision_attention_rope — turns encoder init into `named symbol not found` at
// the first image rather than a clean refusal at load.
//
// GLM-5.3's tower differs from Qwen's in four ops, appended at the bottom of
// this file: RMSNorm instead of LayerNorm in the blocks, a per-head q/k RMSNorm
// between the QKV GEMM and RoPE, a clamped SwiGLU MLP, and an EXACT erf GELU in
// the merger. It reuses vit_rope_deinterleave / vit_softmax_rows /
// vit_scatter_head / vision_spatial_merge / vision_layer_norm (the merger's
// post_projection_norm really is a biased LayerNorm) unchanged.
//
// 🪤 Do NOT "unify" this file with Qwen's by editing vision_gelu to be exact.
// That kernel is tanh-approx BY DESIGN for Qwen (whose vision config declares
// hidden_act gelu_pytorch_tanh) and changing it is a Qwen regression. GLM's
// merger declares gelu_approx "none", so it calls vision_gelu_exact instead.
...
// ═════════════════════════════════════════════════════════════════════════════
// GLM-5.3-Flash additions. Nothing above this line is GLM-specific.
//
// All four accumulate in fp32 and round to BF16 exactly once, matching
// exllamav3's norm_bf16 branch (rope.cu:216-224) — reading BF16, multiplying in
// BF16 and rounding twice drifts visibly across 24 blocks.
// ═════════════════════════════════════════════════════════════════════════════

// ── RMSNorm (in place): y = x * rsqrt(mean(x^2) + eps) * w ──
// GLM's ViT blocks use RMSNorm, not the biased LayerNorm above; the only
// LayerNorm left in this tower is the merger's post_projection_norm, which does
// call vision_layer_norm verbatim.
//
// eps is vision_config.rms_norm_eps = 1e-5 and goes INSIDE the sqrt.
// Grid: (rows, 1, 1)  Block: (256, 1, 1) — block-wide reduction over D.
extern "C" __global__
void vision_rms_norm(
    __nv_bfloat16* __restrict__ X,       // [rows, D] in-place
    const __nv_bfloat16* __restrict__ W, // [D]
    unsigned int rows, unsigned int D,
    float eps
) {
    unsigned int row = blockIdx.x;
    if (row >= rows) return;
    __nv_bfloat16* row_ptr = X + (size_t)row * D;

    float ss = 0.0f;
    for (unsigned int i = threadIdx.x; i < D; i += blockDim.x) {
        float v = bf16_to_f32(row_ptr[i]);
        ss += v * v;
    }
    ...
    if (threadIdx.x == 0) {
        float tot = 0.0f;
        unsigned int nwarps = (blockDim.x + 31u) / 32u;
        for (unsigned int w = 0; w < nwarps; ++w) tot += red[w];
        rms_scale = rsqrtf(tot / (float)D + eps);
    }
    __syncthreads();

    for (unsigned int i = threadIdx.x; i < D; i += blockDim.x) {
        float v = bf16_to_f32(row_ptr[i]) * rms_scale * bf16_to_f32(W[i]);
        row_ptr[i] = f32_to_bf16(v);
    }
}

// ── Per-head RMSNorm over one section of an interleaved QKV buffer ──
// QKV is [rows, row_stride] with row_stride = 3*H*D (3072 on GLM: H=16, D=64).
// `sec_off` is the element offset of the section within a row: 0 for Q, H*D for
// K. V is NEVER normed.
//
// One weight vector W[D] is broadcast across all H heads (attn.py:476-477); the
// checkpoint stores q_norm/k_norm as [64], not [H, 64].
//
// Ordering is load-bearing: q/k-norm runs BETWEEN the QKV GEMM and RoPE
// (rope.cu:245-249 does load_head → apply_norm → apply_rope). Normalising after
// the rotation changes the result.
//
// Grid: (rows, H, 1)  Block: (64, 1, 1) = D.
extern "C" __global__
void vision_rms_norm_heads(
    __nv_bfloat16* __restrict__ QKV,     // [rows, row_stride] in-place
    const __nv_bfloat16* __restrict__ W, // [D], broadcast over heads
    unsigned int rows, unsigned int H, unsigned int D,
    unsigned int sec_off, unsigned int row_stride,
    float eps
) {
    unsigned int row = blockIdx.x;
    unsigned int h   = blockIdx.y;
    if (row >= rows || h >= H) return;
    __nv_bfloat16* head = QKV + (size_t)row * row_stride + sec_off + (size_t)h * D;

    float ss = 0.0f;
    for (unsigned int d = threadIdx.x; d < D; d += blockDim.x) {
        float v = bf16_to_f32(head[d]);
        ss += v * v;
    }
    for (int o = 16; o > 0; o >>= 1) ss += __shfl_down_sync(0xffffffff, ss, o);

    // D = 64 means a 64-thread block, i.e. two warps, so the cross-warp step is
    // required even though a single warp would cover a head on its own.
    __shared__ float red[32];
    unsigned int lane = threadIdx.x & 31u, warp = threadIdx.x >> 5;
    if (lane == 0) red[warp] = ss;
    __syncthreads();
    __shared__ float rmf;
    if (threadIdx.x == 0) {
        float tot = 0.0f;
        unsigned int nwarps = (blockDim.x + 31u) / 32u;
        for (unsigned int w = 0; w < nwarps; ++w) tot += red[w];
        rmf = rsqrtf(tot / (float)D + eps);
    }
    __syncthreads();

    for (unsigned int d = threadIdx.x; d < D; d += blockDim.x) {
        float v = bf16_to_f32(head[d]) * rmf * bf16_to_f32(W[d]);
        head[d] = f32_to_bf16(v);
    }
}

// ── Clamped SwiGLU over a gate|up concat: SRC[rows, 2*N] → DST[rows, N] ──
// SRC row r is [gate(N) | up(N)] contiguous — the layout a single fused
// gate_up GEMM produces.
//
// 🔴 CLAMP BEFORE THE ACTIVATION:
//     g' = min(g, limit);  u' = clamp(u, -limit, limit);  dst = u' * silu(g')
//
// exllamav3 disagrees with itself here: its block-MLP path clamps silu(g)
// instead (activation_kernels.cuh:169-186) while its merger clamps g first, and
// the merger's own source comment says the clamp-first form is the HF
// convention. We follow HF at BOTH sites so the two GLM MLPs cannot drift.
//
// The two readings differ only for g > 10.0005, where the gap is bounded by
// limit - silu(limit) = 10 - silu(10) = 4.54e-4 per gate, i.e. |Δdst| <= 4.54e-3
// once multiplied by a clamped |u| <= 10. Recorded here so it is never
// rediscovered as mystery drift.
//
// No lower clamp on the gate: silu's floor is -0.2785, so a -limit bound is
// inert. silu(x) = x / (1 + expf(-x)), fp32.
//
// Grid: (ceil(rows*N/256), 1, 1)  Block: (256, 1, 1) — one thread per output.
extern "C" __global__
void vision_swiglu_clamp(
    const __nv_bfloat16* __restrict__ SRC, // [rows, 2*N]
    __nv_bfloat16* __restrict__ DST,       // [rows, N]
    unsigned int rows, unsigned int N,
    float limit
) {
    unsigned long long idx =
        (unsigned long long)blockIdx.x * blockDim.x + threadIdx.x;
    if (idx >= (unsigned long long)rows * N) return;
    unsigned long long r = idx / N;
    unsigned int       c = (unsigned int)(idx % N);

    const __nv_bfloat16* src_row = SRC + r * (2ull * N);
    float g = bf16_to_f32(src_row[c]);
    float u = bf16_to_f32(src_row[N + c]);

    g = fminf(g, limit);
    u = fminf(fmaxf(u, -limit), limit);
    float silu_g = g / (1.0f + expf(-g));
    DST[idx] = f32_to_bf16(u * silu_g);
}

// ── EXACT erf GELU (in place): y = 0.5 * x * (1 + erf(x / sqrt(2))) ──
// 🪤 NOT the tanh approximation. `vision_gelu` in this same file is tanh-approx
// BY DESIGN (Qwen's tower declares gelu_pytorch_tanh); GLM-5.3's merger declares
// gelu_approx "none" (glm4v.py:111), so reusing vision_gelu here would be a
// silent ~1e-4/value drift with nothing logged. Both kernels must coexist:
// vision_gelu is hard-required by the shared encoder init table even though
// GLM's forward path never calls it.
//
// Grid: (ceil(n/256), 1, 1)  Block: (256, 1, 1)
extern "C" __global__
void vision_gelu_exact(
    __nv_bfloat16* __restrict__ X,
    unsigned int n
) {
    ...
    float v = bf16_to_f32(X[i]);
    X[i] = f32_to_bf16(0.5f * v * (1.0f + erff(v * INV_SQRT2)));
}
