...
impl TransformerModel {
    ...
    /// DIAGNOSTIC ONLY (`AVAROK_LOGIT_PROBE=1`, default off, no-op otherwise).
    ///
...
/// `AVAROK_LOGIT_PROBE=1` arms [`TransformerModel::logit_probe`]. Read once.
pub(crate) fn logit_probe_enabled() -> bool {
    static ON: std::sync::OnceLock<bool> = std::sync::OnceLock::new();
    *ON.get_or_init(|| std::env::var("AVAROK_LOGIT_PROBE").as_deref() == Ok("1"))
}
