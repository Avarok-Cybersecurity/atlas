    ...
    pub(super) fn decode_verify_graphed_kgamma_dispatch(
        ...
        if need_run {
            ...
            for (layer_idx, layer) in self.layers.iter().enumerate() {
                ...
                // this layer's activation — mirrors verify_b.rs:287 for K=2.
                // Must be inside the graph capture region so the per-layer
                // intermediate (not the final-layer-only post-loop value) is
                // recorded. The post-loop save_dflash_hidden_for_propose call
                // in verify_dflash_step.rs is removed; it was overwriting these
                // correct intermediates with a repeated final-layer hidden.
