...
//! `inherited_targets_w4a4.rs`, and the maintainer rule about hardware-OWNED
//! sources (an override that must not edit gb10; an addition that must not
//! re-declare a gb10 entry point) is `hopper_owned_kernels.rs` — it left this
//! file when #928's prefill twins took it past the 500-line cap. All three
//! read [`INHERITED`] from `support/inherited.rs`, so none can be told about a
//! target the others have not heard of.
