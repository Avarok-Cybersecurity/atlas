...
fn main() {
    ...
    // `avarok_hip` is the strict subset of avarok_scale for the NATIVE-HIP
    ...
    // Vendor, not name. These two cfgs used to key on `hw.starts_with("strix")`
    // and `hw == "strix-hip"`, which made the host build a function of the
    // directory name. The kernel side is not: `prefill_paged_compute.cuh` pins
    // `#define BR64 32` under `#if defined(__SCALE__)`, i.e. for every SCALE
    // target regardless of what its directory is called, and
    ...
    // (`kernels/r9700`, gfx1201) would otherwise compile 32-row kernels and
    // launch them with a 64-row stride, silently and with no build error.
    //
    // So the signal is `[hardware].vendor` in the same `kernels/<hw>/
    // HARDWARE.toml` that avarok-kernels/build.rs reads to pick the compiler:
    ...
    // `apple`), and an unset AVAROK_TARGET_HW still resolves to gb10, i.e.
    // neither cfg.
    ...
    // An unreadable or vendorless file leaves both cfgs unset, which is the
    // NVIDIA behaviour and what a build that cannot see the kernels tree got
    // before this too. A wrong `avarok_scale` would mis-stride prefill on an
    ...
    match hardware_vendor(&hardware_toml).as_deref() {
        Some("amd") => println!("cargo:rustc-cfg=avarok_scale"),
        Some("hip") => {
