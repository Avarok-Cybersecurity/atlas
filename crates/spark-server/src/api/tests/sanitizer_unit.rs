...
//! Unit-level sanitizer tests (direct `sanitize_content_chunk` /
//! `flush_content_sanitizer` calls). Split out of `sanitizer.rs` to keep it
//! under the 500-LoC cap; the whole-stream harness suite stays there.
