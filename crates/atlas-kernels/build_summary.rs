...
/// hopper's `[kernels] overrides` list, which had fifteen entries at that tip
/// and seventeen at the next; the printed field was
/// `find_cu_files(model_kernel_dir).len()` — the per-model/quant directory's
/// own `.cu` count — and had been 14 in round 14 too, while the list was
/// ELEVEN. It never tracked that list, so it could never be the receipt for
/// "did my override land". Now both numbers print, each named for what it is:
///
...
///   itself rather than inheriting from gb10. The three round-15 additions
///   (`paged_decode_*_splitk_hopper.cu`, `ssm_ba_gates_hopper.cu`) land here
///   and moved this number, which is exactly the question the operator asked.
///
...
/// grade the PARSE — an `overrides` table that is absent, empty, or holds a
