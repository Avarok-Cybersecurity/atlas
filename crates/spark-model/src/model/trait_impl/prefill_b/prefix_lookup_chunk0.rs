...
//! The chunk-0 prefix lookup, split out of `prefix_lookup.rs` to keep it
//! under the 500-line cap.

use super::*;

impl TransformerModel {
    /// The `chunk_start == 0` arm of [`Self::prefill_b_prefix_lookup`] --
    /// the lookup itself. Split out of `prefix_lookup.rs` to keep it under
    /// the 500-line cap; `bs` is the caller's `kv_cache.block_size()`.
    #[allow(clippy::too_many_arguments)]
    pub(super) fn prefill_b_prefix_lookup_chunk0(
        &self,
        tokens: &[u32],
        ...
        if ep_active && !reserved {
            ...
            prefix_match = if agreed > 0 {
                self.prefix_cache
                    .lookup(&tokens[..agreed], bs, seq.session_hash, seq.adapter_id)
            } else {
