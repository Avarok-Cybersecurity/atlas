...
Mixture-of-Experts is where the Avarok supported-model matrix gets most of its diversity. Qwen3.5 routes 128 experts top-10. MiniMax-M2.7 routes 256 experts top-8 with **sigmoid** gating (not softmax). Gemma-4 has shared experts. Nemotron-H has an MoE-only layer variant (no mixer, just routing + FFN). The engineering problem is making all these shapes share one kernel pipeline without losing per-shape efficiency.
...
Avarok's `moe_prefill.cu` kernel takes the "sort tokens by expert, then one GEMM per expert group" approach. The sort is parallel — dispatch tables are computed in a single pre-kernel pass. The GEMM itself uses the same 16×8×16 BF16 MMAs as attention, one CTA per expert-bucket.
...
Avarok's `minimax_moe` layer uses:
...
The shared expert is a standard dense FFN run alongside the routed experts. Avarok fuses the shared FFN with the gather step in the decode kernel (`moe_shared_expert_fused_fp8.cu`) to save a memory round-trip.
...
The end result on a 256-expert MoE at batch=80: Avarok at 8.43 ms vs PyTorch at 32.65 ms — **3.87×**. That's the biggest single-kernel win in the benchmark table.
