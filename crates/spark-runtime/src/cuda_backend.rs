...
unsafe extern "C" {
    ...
    /// Build a 128-byte TMA descriptor for a tiled global tensor.
    ///
    /// `tensor_map` MUST point at 128 bytes aligned to 64. `rank` is 1..=5, and
    /// `global_dim`/`global_strides`/`box_dim`/`element_strides` are read as
    /// arrays of that rank — `global_strides` carries rank-1 entries (the
    /// innermost stride is implicit and must be the element size), in BYTES.
    ///
    /// The descriptor is consumed by `cp.async.bulk.tensor` inside the kernel,
    /// which is what makes TMA loads bounds-checked and address-generated by
    /// hardware rather than by us. Available on sm_90+; VERIFIED to compile and
    /// run on GB10/sm_121a with CUDA 13.0.
