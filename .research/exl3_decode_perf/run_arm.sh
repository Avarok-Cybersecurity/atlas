...
[ "${NOLAUNCH:-0}" = 1 ] || setsid $D/serve_exl3_fix.sh 8890 > "$LOG" 2>&1 < /dev/null &
for i in $(seq 1 900); do
  if curl -s -m 2 http://127.0.0.1:8890/v1/models >/dev/null 2>&1; then echo "READY after ~${i}s"; break; fi
  if ! pgrep -f "serv[e] --model-from-path.*8890" >/dev/null; then echo "SERVER EXITED"; tail -20 "$LOG" | cut -c1-200; exit 1; fi
  sleep 1
done
python3 -u $D/measure_decode.py --port 8890 --repeats 3 --max-tokens 300 > $D/measure_${ARM}.txt 2>&1
cat $D/measure_${ARM}.txt
# greedy sample for output-equality check across arms (non-streaming, 200 tokens)
curl -s -m 600 http://127.0.0.1:8890/v1/chat/completions -H 'Content-Type: application/json' -d '{
...
pkill -f "serv[e] --model-from-path.*8890"; sleep 6; pgrep -af "spark serv[e]" | cut -c1-40; echo "arm $ARM done"
