...
impl MtpHead {
    ...
    pub(crate) fn forward_one(
        ...
        dbg::dump_fc(ctx.gpu, hidden, h, stream);
        ...
        dbg::dump_attn(ctx.gpu, attn_out, gate_ptr, (nq * hd) as usize, stream);
        ...
        dbg::dump_final(ctx.gpu, target_hidden, final_normed, logits, h, v, stream);

        // 13. Argmax (grammar-masked CPU path / GPU fast path) — hoisted to
        // forward_argmax.rs to keep this file under the 500 LoC cap.
        let token_id = self.argmax_draft_token(
            logits,
            v,
            h,
            position,
