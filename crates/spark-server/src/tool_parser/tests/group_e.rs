...
// the historical backfill home) because `group_a` is currently disabled —
// it references the renamed-and-now-private `parse_minimax_xml_call` and is
// commented out of `tests.rs` pending that cleanup. Putting the regression
// here keeps it COMPILED and RUNNING.
// ────────────────────────────────────────────────────────────────────────
