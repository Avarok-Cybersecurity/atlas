...
fn run(f: &Fx, w: &Layer0, r: &str) -> Run {
    ...
    let (attn_pre, attn_post, attn_comb) =
        hc_mixes(&h_in, tokens, f.hc, f.dim, &w.attn_fn, &w.attn_scale, &w.attn_base, f.iters, f.hc_eps, f.norm_eps);
    let attn_in = rms_norm(&hc_pre(&h_in, &pre_mix_in, tokens, f.hc, f.dim), &w.attn_norm, tokens, f.dim, f.norm_eps);
    let h_mid = hc_post(&attn_out, &h_in, &attn_post, &attn_comb, tokens, f.hc, f.dim);
    let (ffn_pre, ffn_post, ffn_comb) =
        hc_mixes(&h_mid, tokens, f.hc, f.dim, &w.ffn_fn, &w.ffn_scale, &w.ffn_base, f.iters, f.hc_eps, f.norm_eps);
    // the FFN collapses with the mix THIS block's attention produced
    let ffn_in = rms_norm(&hc_pre(&h_mid, &attn_pre, tokens, f.hc, f.dim), &w.ffn_norm, tokens, f.dim, f.norm_eps);
    let h_out = hc_post(&ffn_out, &h_mid, &ffn_post, &ffn_comb, tokens, f.hc, f.dim);
    Run { tokens, attn_pre, attn_post, attn_comb, attn_in, h_mid, ffn_pre, ffn_post, ffn_comb, ffn_in, h_out }
}
...
fn attention_mixes_match() {
    ...
    for r in &f.regimes {
        let s = run(&f, &w, r);
        check_capture(&format!("{r}.L0.attn_pre"), &as_f64(&s.attn_pre), &f.g.tensor(r, "L0.attn_pre"), F32_TOL, F32_TOL);
        check_capture(&format!("{r}.L0.attn_post"), &as_f64(&s.attn_post), &f.g.tensor(r, "L0.attn_post"), F32_TOL, F32_TOL);
        check_capture(&format!("{r}.L0.attn_comb"), &as_f64(&s.attn_comb), &f.g.tensor(r, "L0.attn_comb"), F32_TOL, F32_TOL);
    }
}
...
fn attention_input_matches_within_bf16() {
    ...
    for r in &f.regimes {
        ...
        check_capture(&format!("{r}.L0.attn_in"), &as_f64(&s.attn_in), &gt, bf16_tol(&gt), BF16_CK_REL);
    }
}
...
fn ffn_mixes_match_through_hc_post() {
    ...
    for r in &f.regimes {
        ...
        check_capture(&format!("{r}.L0.ffn_pre"), &as_f64(&s.ffn_pre), &f.g.tensor(r, "L0.ffn_pre"), F32_TOL, F32_TOL);
        check_capture(&format!("{r}.L0.ffn_post"), &as_f64(&s.ffn_post), &f.g.tensor(r, "L0.ffn_post"), F32_TOL, F32_TOL);
        check_capture(&format!("{r}.L0.ffn_comb"), &as_f64(&s.ffn_comb), &f.g.tensor(r, "L0.ffn_comb"), F32_TOL, F32_TOL);
    }
}
...
fn ffn_input_matches_within_bf16() {
    ...
    for r in &f.regimes {
        ...
        check_capture(&format!("{r}.L0.ffn_in"), &as_f64(&s.ffn_in), &gt, bf16_tol(&gt), BF16_CK_REL);
    }
}
...
fn block_output_and_next_pre_mix_match() {
    ...
    for r in &f.regimes {
        ...
        check_capture(&format!("{r}.L0.h_out"), &as_f64(&s.h_out), &gt, bf16_tol(&gt), BF16_CK_REL);
        // Block.forward returns ffn_pre as the NEXT block's pre_mix
        check_capture(&format!("{r}.L0.pre_mix_out"), &as_f64(&s.ffn_pre), &f.g.tensor(r, "L0.pre_mix_out"), F32_TOL, F32_TOL);
    }
}
