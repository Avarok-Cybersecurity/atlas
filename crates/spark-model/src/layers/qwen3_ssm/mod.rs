...
pub struct Qwen3SsmLayer {
    ...
    /// DFlash verify: batched conv that also writes per-token conv_state
    /// intermediates (causal_conv1d_update_prefill_inter). NULL → fall back to
    /// the per-token phase-1 loop.
    conv1d_prefill_inter_k: KernelHandle,
...
impl TransformerLayer for Qwen3SsmLayer {
    ...
    fn prefill_phase1_verify(
        ...
        // inter base + stride (FP32 elements) from the contiguous conv-intermediate
        // pool: conv_state_intermediates[t] = base + t * (conv_dim*d_conv) floats.
