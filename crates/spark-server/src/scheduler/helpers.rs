...
// Token-level loop detectors (`detect_thinking_token_loop`,
// `detect_content_token_loop`, the digit-normalized variants, and the
// shared vLLM-anchored period matcher) were moved to `loop_detect.rs`
// to keep this file ≤500 LoC. They are re-exported below so existing
// `super::*` / `helpers::*` call sites are unaffected.
mod loop_detect;
pub use loop_detect::*;
