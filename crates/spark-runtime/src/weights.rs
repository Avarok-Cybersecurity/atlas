...
impl SafetensorsLoader {
    ...
    ///
    /// `pub` for two reasons: residency can be PROVEN against a real checkpoint
    /// index without collectives (see `spark-model/tests/glm53_ep_residency.rs`),
    /// and the out-of-index sidecar registration (spark-model
    /// `register_exl3_sidecar_shards`) reuses it so a sidecar cannot smuggle in
    /// a tensor these rules withheld (fast_weights/skip.rs is identical).
