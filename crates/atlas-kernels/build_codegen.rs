...
/// Generate the target_ptx.rs source with per-target PTX constants and lookup functions.
pub(super) fn generate_target_ptx_rs(targets: &[Target], all_modules: &[Vec<(String, String)>]) -> String {
    ...
    for (idx, (target, modules)) in targets.iter().zip(all_modules.iter()).enumerate() {
        ...
        for (stem, module_name) in modules {
            let const_name = format!("{}{}_PTX", prefix, module_name.to_uppercase());
            g.push_str(&format!(
                "pub const {const_name}: &str = \
                 include_str!(concat!(env!(\"ATLAS_PTX_DIR\"), \"/t{idx}__{stem}.ptx\"));\n"
            ));
        }
        g.push('\n');

        // Per-target ptx_modules function
        ...
        g.push_str(&format!(
            "/// PTX modules for target: ({}, {}, {}).\n",
            target.hw, target.model, target.quant
        ));
        g.push_str(&format!(
            "pub fn {fn_name}() -> Vec<(&'static str, &'static str)> {{\n    vec![\n"
        ));
    ...
    // all_ptx_sets() function — always generated
    g.push_str(
