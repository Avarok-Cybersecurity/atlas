...
use crate::tool_parser::FunctionDefinition;
    ...
    fn salvage_dedupes_identical_args_extracted_twice() {
        ...
        let calls = salvage(&content, &[write_tool()]);
        assert_eq!(calls.len(), 1, "same (name, args) must dedupe — got {calls:#?}");
    }
    ...
    fn bare_invocation_inline_mention_does_not_fire() {
        ...
        assert!(calls.is_empty(), "inline mentions must not salvage: got {calls:#?}");
    }
...
"#;
    ...
    fn file_content_pair_skipped_when_no_write_tool() {
        let content =
            "<file>/tmp/x</file><content>body</content>";
        let calls = salvage(content, &[bash_tool()]);
        assert!(
            calls.is_empty(),
            "no write tool declared → no salvage"
        );
    }
...
"#;
    ...
    fn file_content_pair_skipped_when_distance_too_far() {
        ...
        let content = format!(
            "<file>/tmp/x.rs</file>{filler}<content>body</content>"
        );
        let calls = salvage(&content, &[write_tool()]);
        assert!(
            calls.is_empty(),
