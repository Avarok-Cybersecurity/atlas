...
avarokctl run qwen3.6-35b-a3b-fp8-mtp
```

The script downloads a prebuilt `avarokctl`, verifies its checksum, and installs it to `~/.local/bin` — no Python, no Rust toolchain. Prefer not to pipe curl into a shell? `cargo install avarokctl` does the same from source. Every runnable model maps to a recipe in [avarok-recipes](https://github.com/Avarok-Cybersecurity/atlas-recipes), so the catalogue cannot list a model we do not ship.
...
- **Add or tune a recipe** in [avarok-recipes](https://github.com/Avarok-Cybersecurity/atlas-recipes) — recipes are the model SSOT.
...
- **Follow along** on the [blog](https://blog.atlascybernetics.ai) and on [X @AtlasInferenceX](https://x.com/AvarokInferenceX).
