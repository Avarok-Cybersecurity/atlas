...
// Atlas WY-Chunkwise Gated Delta Rule — K=16 verification (DFlash γ).
//
// Generalization of gated_delta_rule_wy4.cu to K=16 tokens. DFlash with
// γ=16 produces 15 drafts + 1 prefix-bonus position, so the K=γ verify
// path runs 16 tokens through every SSM layer in one fused pass. (The K=17
// sibling gated_delta_rule_wy17.cu handles the γ+1 / draft-cap variant.)
//
...
//   kdots[120]                             = 0.47 KB
//   gate/beta scalars[2·16]                = 0.1 KB
//   warp_sums[4]                           = 16 B
//   ────────────────────────────────────────
//   Total                                  ≈ 16.6 KB  (SM_120 cap: 100 KB)
//
...
//
// `h_state` itself becomes the K-1=15th (final) intermediate.
