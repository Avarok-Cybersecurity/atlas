...
//! that touches nothing its gate measures; a record from off the head's
//! history, or from before a perf-path change, does not stand.
...
#[test]
fn a_record_stands_across_harmless_commits_and_falls_to_a_perf_path_or_a_foreign_branch() {
    ...
    // NEGATIVE CONTROL: a commit on a side branch that the head does not
    // descend from — its record is off this history, whatever it diffs to.
