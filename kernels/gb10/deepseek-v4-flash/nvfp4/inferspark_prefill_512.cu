...
extern "C" __global__ void inferspark_prefill_512(
    ...
    const float* __restrict__ sinks  // [num_q_heads] per-head sink logit (denominator only); nullptr = no sink. FP32: MlaWeights.attn_sink is the checkpoint-native FP32 buffer (indexed as float; a bf16 index would 2-byte-stride the fp32 elems and corrupt even heads).
) {
