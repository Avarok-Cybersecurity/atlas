...
impl TransformerModel {
    ...
    /// `AVAROK_MTP_EP_BATCH_VERIFY=1` lets the batched verify sweep run under EP.
    ///
    ...
    pub(super) fn ep_batch_verify_enabled() -> bool {
        ...
        *ON.get_or_init(|| std::env::var("AVAROK_MTP_EP_BATCH_VERIFY").as_deref() == Ok("1"))
    }
    ...
    pub(super) fn can_batch_verify_dispatch(&self, ks: &[usize]) -> bool {
        ...
        (2..=crate::layer::VERIFY_WY_TABLE_SEQS).contains(&n)
            ...
            // rather than assumed.
            //
            // 🪤 Requires AVAROK_EP_PROTOCOL=v2 on BOTH ranks. The command is
    ...
    pub(in crate::model) fn decode_verify_batched_dispatch(
        ...
        if let Some(graph) = replay {
            ...
            let stage_timing = {
                static ON: std::sync::OnceLock<bool> = std::sync::OnceLock::new();
                *ON.get_or_init(|| {
                    std::env::var("AVAROK_HC_VERIFY_STAGE_TIMING").as_deref() == Ok("1")
                })
            };
                            ...
                            SAID.call_once(|| {
                                tracing::info!(
                                    ...
                                    "hc batched verify: attention projections + brackets BATCHED \
                                     across sequences (AVAROK_HC_ATTN_CORE_BATCHED=0 restores \
                                     per-sequence)"
                                );
                            });
                        }
                            ...
                            SAID.call_once(|| {
                                tracing::info!(
                                    ...
                                    "hc batched verify: attention FFN sublayer BATCHED across \
                                     sequences (AVAROK_HC_ATTN_FFN_BATCHED=0 restores per-sequence)"
                                );
                            });
                        }
                    }
