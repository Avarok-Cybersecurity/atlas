...
//! Tests for [`super`] — the GPU-compatibility preflight.
//!
//! Split into its own file for the repository's 500-LoC cap when the
//! SM-count cross-check (#928) was added. They stay a `#[cfg(test)] mod tests`
//! of `arch_preflight` via `#[path]`, so `super::` still reaches the private
//! `DeviceQuery` seam the ordering contract is asserted through — a `tests/`
//! integration binary could not.
...
impl DeviceQuery for FakeDriver {
    ...
    fn sm_count(&self, _ordinal: usize) -> Result<u32> {
        Ok(132)
    }
...
/// ★ THE SILENT CONSTANT, pinned. A build whose declared SM count does not
/// match the card must say so at boot, naming BOTH numbers.
///
/// Oracle: #928. `atlas_core::device::sm121::NUM_SMS = 48` — the GB10
/// constant — was compiled into the H100 attention dispatch, so
/// `paged_decode_attn_fp8` ran `grid=(24,1,1)` on a 132-SM card for an
/// entire campaign with nothing in any log saying so
/// (`ATTN-DECODE-SPLITK-ATTRIBUTION.md`). This is the line that would have.
#[test]
fn a_device_with_a_different_sm_count_is_named_in_the_warning() {
    let warning = check_sm_count(132, 48).expect("48 != 132 is a mismatch");
    assert!(warning.contains("132"), "{warning}");
    assert!(warning.contains("48"), "{warning}");
    assert!(warning.contains("sm_count"), "{warning}");
    // It is a WARNING, not a refusal: a pessimistic constant costs a
    // suboptimal grid and nothing else, because the split-K workspace is
    // allocated from the same constant the grid is derived from.
    assert!(warning.contains("serving is unaffected"), "{warning}");
}

/// An agreeing device says nothing at all — a boot line that warned on
/// every correct build would be a boot line nobody reads.
#[test]
fn a_matching_sm_count_is_silent() {
