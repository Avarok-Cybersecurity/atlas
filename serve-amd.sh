...
#
# A binary built with AVAROK_TARGET_MODEL='*' (build-amd.sh's default) carries
...
export AVAROK_W4A16_DP4A=1         # int8-DP4A decode GEMV
#
# NOT set, though every earlier Strix doc lists it: AVAROK_FORCE_GLOBAL_GDN=1.
...
export AVAROK_SSM_TAIL_MIDCHUNK=1  # capture the mid-chunk SSM tail state
export AVAROK_MTP_GATE_REPROBE=64  # re-probe the MTP accept gate every 64 tokens
#
...
#   AVAROK_SSM_TAIL_PROTECT=1   renamed 2026-08-05 to the opt-OUT
#                              AVAROK_DISABLE_SSM_TAIL_PROTECT; the lease is now
#                              on by default, so setting the old name does
#                              nothing and the behaviour is unchanged.
#   AVAROK_MTP_DRAFTER_PREFILL=1 } "OBSOLETE and IGNORED — MTP drafter prefill
#   AVAROK_MTP_CARRY_DRAFTER=1   } and cross-turn carry are ON by default"
#                              (spark_model::model::drafter_context). To turn
#                              them off you now set AVAROK_NO_MTP_DRAFTER_CONTEXT=1.
...
export AVAROK_KV_EXTERNAL_RESERVE_GB="${AVAROK_KV_EXTERNAL_RESERVE_GB:-6}"
...
# 0 would still turn it ON. Set AVAROK_NO_GDN_FP8_PREFILL=0 to genuinely disable
# it — this branch unsets the variable rather than passing a falsy value:
if [ "${AVAROK_NO_GDN_FP8_PREFILL:-1}" = "0" ]; then
  unset AVAROK_NO_GDN_FP8_PREFILL
else
  export AVAROK_NO_GDN_FP8_PREFILL=1
fi

if [ "$HW" = "strix-hip" ]; then
  # The HIP shims (libcuda/libcudart/libcublasLt) are built into avarok-kernels'
  # OUT_DIR; the loader needs them plus the AMD runtime at /opt/rocm/lib.
  SHIM=$(ls -dt target/release/build/avarok-kernels-*/out 2>/dev/null | head -1)
