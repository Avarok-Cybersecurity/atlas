...
//!
//! WHY THIS IS NOT THE REVERTED PARALLEL PREWARM
//! ---------------------------------------------
//! `7cf68075` made the prewarm itself four-way parallel and `37228e85`
//! reverted it: four workers contending on the shared `Arc<GrammarData>`
//! made cold latency WORSE on the H100 (5.28-6.57 s vs 4.82 s). This
//! keeps the prewarm strictly serial — one worker, exactly as
//! `prewarm_max_threads = 1` in `engine.rs` sets it — and only moves it
//! off the request thread. No two workers ever touch one grammar.
//!
