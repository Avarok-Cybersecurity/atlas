...
//! Fused gate+up / silu+down expert GEMV wrappers with the shared expert
//! folded into an extra `blockIdx.y` slot (NVFP4 / FP8 / BF16 variants).
//!
//! Split out of `moe_expert.rs` to keep both files under the 500 LoC cap.
//! Pure thin kernel-launch wrappers; re-exported alongside `moe_expert::*`
//! via `ops.rs`.
