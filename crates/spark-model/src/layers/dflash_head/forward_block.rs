...
impl BlockDiffusionDraftHead {
    pub(super) fn forward_block(
        ...
        // Extracted to `forward_block_step0.rs` for the 500-LoC budget.
        self.forward_block_step0_fc_projection(
            ctx,
            &super::forward_block_step0::Step0Args {
                last_token,
                position,
                stream,
                ctx_base_ptr,
        ...
        if debug_dump {
            ...
            dump_bf16(
                "final.logits[noise1]",
                self.scratch.logits.offset(logits_vocab * bf16),
                10,
            )?;
        }
