...
impl GdnFlags {
    ...
    /// The reading used when the CLI never set anything: the compiled target's
    /// declaration, with the environment overriding.
    ///
    ...
    /// per-sequence launches), `gb10` declares it OFF, unchanged. It used to be
    /// `ATLAS_SSM_BATCHED_RECURRENT=1` in an H100 launch script outside this
    /// repository, which is the structure the 2026-09-11 maintainer review
    /// asked for. `ATLAS_SSM_BATCHED_RECURRENT` still overrides, and `=0` now
    /// means OFF rather than ON (see `layers::ops::target_defaults`).
    ///
