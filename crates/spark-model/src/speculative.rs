...
/// `AVAROK_EP_PROTOCOL=v2` for the same reason batched decode does.
///
...
/// drafts (#1026) unable to reach `AVAROK_LOOKUP_WIDTH` on a multi-rank serve:
...
/// commands: the preamble `seq_id` routes it, so no `AVAROK_EP_PROTOCOL=v2`
