...
//
// Ported UNMODIFIED into PR #604 from Ronald R. Stesiak's PR #648, commit
// acaf9533. Only the dispatch differs on this branch: propose batches rows
// across sequences (M = gamma * n_seq), so the M<=8 guard admits C=1 and the
// tile GEMMs still serve C>=2. See `fp8_rt_enabled()` and its call sites.
//
