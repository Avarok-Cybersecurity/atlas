...
//! `TransformerLayer` impl for `Qwen3SsmLayer` — thin delegations to the
//! per-file inherent `_inner` methods. Split from `mod.rs` for the 500-LoC cap.
...
impl TransformerLayer for Qwen3SsmLayer {
    ...
    fn prefill_phase1_verify(
        ...
        self.prefill_phase1_verify_inner(
            hidden,
            residual,
            num_tokens,
            state,
