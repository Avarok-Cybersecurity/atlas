...
fn main() -> Result<()> {
    ...
    let k_pre: KernelHandle = gpu.kernel("hyper_connection", "hc_pre")?;
    ...
    println!(
        "\n{:>3} {:9} {:5} {:10} {:>11} {:>11} {:>11} {:>7}  verdict",
        "L", "regime", "site", "stage", "E (gpu-f32)", "B (bf16fl)", "|ref|max", "E/B"
    );
    // A raw `comb` row above the floor is NOT a failure if its `comb@colproj` twin sits at the
    // floor: that pair is the signature of the one known deviation — `hc_pre`'s final exact column
    // projection, absent from GLM's eps-ending Sinkhorn. It is named, bounded at ~hc_eps, and
    // attributed, not waived. Any OTHER above-floor row is a real failure.
    let colproj_ok: std::collections::BTreeSet<(usize, &str, &str)> = rows
        .iter()
        .filter(|r| r.stage == "comb@colproj" && r.b > 0.0 && r.e <= r.b)
        .map(|r| (r.layer, r.regime, r.site))
        .collect();
    ...
    for r in &rows {
        ...
        if !ok && r.stage == "comb" && colproj_ok.contains(&(r.layer, r.regime, r.site)) {
            ok = true;
            known += 1;
            label = "known: colproj (~hc_eps)";
        }
        if !ok {
            fails += 1;
        }
        ...
        println!(
            "{:>3} {:9} {:5} {:10} {:>11.4e} {:>11.4e} {:>11.4e} {:>7.3}  {}",
            r.layer, r.regime, r.site, r.stage, r.e, r.b, r.mag, ratio, label
        );
    }
    println!(
        "\n{} rows, {} unexplained above the bf16 activation floor, {} attributed to the \
         hc_pre column projection, worst E/B = {:.3}",
        rows.len(),
        fails,
        known,
        worst_ratio
    ...
    println!(
        "mHC gate PASS — Atlas hc_pre/hc_post reproduce Glm5NextTextHyperConnection and the \
         decoder layer's residual write. ONE deviation, attributed: hc_pre's final exact column \
         projection (absent from GLM's eps-ending Sinkhorn), ~hc_eps on every comb entry."
    );
    println!(
