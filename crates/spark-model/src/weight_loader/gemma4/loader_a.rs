        ...
        for i in 0..config.num_hidden_layers {
            ...
            if tp_size > 1 {
                let (qp, _, _) = shard_dense_bf16(
                    q_dense.weight, q_out_dim, h,
                    TpShardKind::ColumnParallel, tp_rank, tp_size, gpu,
                )?;
                if qp != q_dense.weight { gpu.free(q_dense.weight)?; }
                q_dense.weight = qp;
                let (kp, _, _) = shard_dense_bf16(
                    k_dense.weight, kv_out_dim, h,
                    TpShardKind::ColumnParallel, tp_rank, tp_size, gpu,
                )?;
                if kp != k_dense.weight { gpu.free(k_dense.weight)?; }
                k_dense.weight = kp;
                if v_aliases_k {
                    ...
                    let (vp, _, _) = shard_dense_bf16(
                        v_dense.weight, kv_out_dim, h,
                        TpShardKind::ColumnParallel, tp_rank, tp_size, gpu,
                    )?;
                    if vp != v_dense.weight { gpu.free(v_dense.weight)?; }
                    v_dense.weight = vp;
                }
                let (op, _, _) = shard_dense_bf16(
                    o_dense.weight, h, q_out_dim,
            ...
            let (q_nvfp4_opt, k_nvfp4_opt, v_nvfp4_opt) = if bf16_attn {
                ...
                let q =
                    quantize_to_nvfp4(&q_dense, q_out_dim, h, gpu, absmax_k, quantize_k, stream)?;
                let k =
                    quantize_to_nvfp4(&k_dense, kv_out_dim, h, gpu, absmax_k, quantize_k, stream)?;
                let v =
                    quantize_to_nvfp4(&v_dense, kv_out_dim, h, gpu, absmax_k, quantize_k, stream)?;
            ...
            let o_nvfp4 =
                quantize_to_nvfp4(&o_dense, h, q_out_dim, gpu, absmax_k, quantize_k, stream)?;
            ...
            let mut ffn_layer = DenseFfnLayer::new_with_activation(
                ffn_weights,
                FfnActivation::GeLU,
                gpu,
            )?;
