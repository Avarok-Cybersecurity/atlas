...
//
// Decode MoE, N-row variant (the K-row verify arm, #1060): the batch3 kernels
// with the row count as a launch argument instead of a constant. Same block
// per (row, expert slot), same per-thread reduction order, so every row is
// bit-identical to the batch3 row. Grid y = num_tokens * (top_k + 1): routed
// slots first, then one shared-expert block row per token.

// Avarok Fused MoE Expert+Shared GEMV — K=3 multi-token batch variant.
//
...
//
// Grid: gate_up_batchn  (ceil(N/8), 3*(top_k+1), 2)
//       silu_down_batchn (ceil(N/8), 3*(top_k+1), 1)
...
__device__ __constant__ float E2M1_LUT_BATCHN[16] = {
...
extern "C" __global__ void moe_expert_gate_up_shared_batchn(
    ...
    if (threadIdx.x < 16) s_lut[threadIdx.x] = E2M1_LUT_BATCHN[threadIdx.x];
    __syncthreads();
...
extern "C" __global__ void moe_weighted_sum_blend_batchn(
