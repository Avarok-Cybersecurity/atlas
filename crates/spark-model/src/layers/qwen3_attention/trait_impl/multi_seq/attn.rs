...
impl Qwen3AttentionLayer {
    ...
    pub(super) fn ms_phase_rope(&self, c: &MultiSeqCtx<'_>, meta: AttnMetadataDev) -> Result<()> {
        ...
        // Q/K differently than the serial decode path — measured on Laguna-XS
        // DFlash as verify logits diverging from serial at every position
        // (the seam commit flips '\n' → 'To' and temp-0 spec output walks a
        // different-but-coherent answer). Bit-parity for non-yarn layers is
        // unchanged.
        if n > 1
