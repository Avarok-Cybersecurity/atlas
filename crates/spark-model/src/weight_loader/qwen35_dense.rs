...
use load_linear_attn::{LinearAttnKernels, load_linear_attention_layer};
use spark_runtime::weights::WeightDtype;

use crate::weight_map::{
    AttentionWeights, DenseWeight, MtpWeights, Nvfp4Variant, dense, dense_auto,
    detect_nvfp4_variant, load_dense_ffn, load_kv_scales, load_mtp, quantize_to_nvfp4,
    quantized_auto,
};
        ...
        for (i, lt) in layer_types.iter().enumerate() {
            ...
            match lt {
                ...
                LayerType::LinearAttention => {
                    let kernels = LinearAttnKernels {
                        absmax_k,
                        quantize_k,
                        bf16_to_fp8_k,
                    };
                    let layer = load_linear_attention_layer(
                        store,
                        config,
                        gpu,
                        stream,
