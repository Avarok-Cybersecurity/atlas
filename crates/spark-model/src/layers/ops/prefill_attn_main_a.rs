...
pub fn prefill_attention_64(
    ...
    // (atlas_scale) is set for every `vendor = "amd"`/`"hip"` target, matching
    // the kernel's own `__SCALE__`/`__HIP_PLATFORM_AMD__` gate rather than a
    // directory name; NVIDIA keeps 64 (byte-identical). See the @human-review note in inferspark_prefill.cu.
