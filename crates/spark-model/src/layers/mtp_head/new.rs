...
impl MtpHead {
    pub fn new(
        ...
        // ATLAS_MTP_DRAFTER_PREFILL: dedicated batched-prefill scratch, allocated
        // only when the env is set (PCND). Dedicated (not aliased) so the pass
        // has no aliasing hazards against target buffers.
        ...
        Ok(Self {
            ...
            moe_silu_mul_k,
            moe_weighted_sum_blend_k,
            // Batched BF16 GEMM for drafter prefill; 0-handle when the target's
            // kernel set lacks it (prefill then no-ops).
