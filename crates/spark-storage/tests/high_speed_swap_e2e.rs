...
fn tempdir(name: &str) -> PathBuf {
    let p = std::env::temp_dir().join(format!(
        "atlas-hss-e2e-{name}-{}", std::process::id()
    ));
...
fn run_in_hbm_reference(ctx: &CudaCtx, q: &[bf16], k: &[bf16], v: &[bf16]) -> Vec<bf16> {
    ...
    copy_h_to_d_async(bt_dev.ptr, bt.as_ptr() as *const c_void, SEQ_BLOCKS as usize * 4, ctx.stream).unwrap();
    copy_h_to_d_async(counts_dev.ptr, counts.as_ptr() as *const c_void, 4, ctx.stream).unwrap();
    attn.begin_step(ctx, 1).unwrap();
    let (s_blk, s_tok, s_kvh) = attn.paged_strides();
    attn.step_tile(ctx, q_dev.ptr, k_dev.ptr, v_dev.ptr, bt_dev.ptr, counts_dev.ptr, 1, s_blk, s_tok, s_kvh, BLOCK_SIZE as i32).unwrap();
...
fn orchestrator_multi_tile_with_eviction() {
    ...
    let total = SEQ_BLOCKS as usize * BLOCK_SIZE as usize * NUM_KV_HEADS as usize * HEAD_DIM as usize;
    ...
    hss.attend_layer(&ctx, 0, &seq_blocks, q_dev.ptr, out_dev.ptr).unwrap();
    ...
    assert!(max_d < 1e-2, "orchestrator output diverged from reference: {max_d}");
