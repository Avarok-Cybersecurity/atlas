...
//! `AVAROK_BUF_CKSUM`: a per-buffer sum / sum-of-squares / sum-of-abs dump of
//! the whole arena, for bisecting where a forward pass starts producing
//! garbage. It copies every intermediate buffer back to the host and is only
//! ever reached from a debug path.

use super::super::BufferArena;
