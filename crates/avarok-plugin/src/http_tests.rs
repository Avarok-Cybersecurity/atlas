...
// ---- ITL per AIPerf, the server's raw window, and the jitter instrument ----
...
fn server_raw_timing_components_are_captured_and_the_server_itl_is_derived() {
    ...
    let from_total =
        (out.server_total_time_ms.unwrap() - out.server_ttft_ms.unwrap()) / 9.0;
    ...
    assert_eq!(bare.server_tpot_ms(), None, "no raw window → no server ITL, not 0");
    ...
    assert_eq!(one_token.server_tpot_ms(), None, "undefined below two tokens");
}
...
async fn arrival_gaps_are_per_read_and_a_stall_shows_in_the_tail() {
    ...
    let out = chat_stream(&target, &serde_json::json!({}), Duration::from_secs(5))
        .await
        .expect("stream");
    assert_eq!(out.completion_tokens, 14, "10 arrivals, 4 of them bursts of 2");
