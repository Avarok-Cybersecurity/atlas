...
#
# Start Sehyo/Qwen3.5-35B-A3B-Turbo3 + LiteLLM proxy
# — thinking mode DISABLED globally
# — MTP speculative K=2 (num-drafts 1)
# — Turbo3 quantization for KV cache and MTP (22% smaller than Turbo4)
#
# Usage: ./start-qwen35-turbo3.sh
#
# Architecture:
...
MODEL_ID="Sehyo/Qwen3.5-35B-A3B-NVFP4"
...
echo "   Speculative: K=2 (num-drafts 1, turbo3)"

"$SPARK_BIN" serve "$MODEL_ID" \
    --port "$ATLAS_PORT" \
    --max-seq-len 60000 \
    --kv-cache-dtype turbo3 \
...
echo "   Model: qwen3.5-35b-turbo3"
echo "   KV Cache: turbo3 (22% smaller than turbo4, 3-bit WHT + Lloyd-Max)"
echo "════════════════════════════════════════════"
