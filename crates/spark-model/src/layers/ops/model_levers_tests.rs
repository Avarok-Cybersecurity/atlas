...
/// `AVAROK_GDN_FLA_UNDER_PREFIX_CACHE` is an EXACT "1" opt-in, not a truthy one:
...
fn the_gdn_fla_prefix_cache_kill_switch_is_an_exact_opt_in() {
    ...
    assert!(!resolve(&[("AVAROK_GDN_FLA_UNDER_PREFIX_CACHE", "true")]).gdn_fla_under_prefix_cache);
    assert!(!resolve(&[("AVAROK_GDN_FLA_UNDER_PREFIX_CACHE", "0")]).gdn_fla_under_prefix_cache);
    assert!(resolve(&[("AVAROK_GDN_FLA_UNDER_PREFIX_CACHE", "1")]).gdn_fla_under_prefix_cache);
}
