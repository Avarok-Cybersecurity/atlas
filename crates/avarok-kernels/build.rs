...
fn main() {
    ...
    println!("cargo:rerun-if-changed=../avarok-core/src/elf_symbols.rs");
    for (idx, modules) in all_target_modules.iter().enumerate() {
        for (stem, module_name) in modules {
            ...
            if elf_symbols::defined_function_symbols(&bytes).is_some_and(|k| k.is_empty()) {
                println!(
                    "cargo:warning=avarok-kernels: {module_name} compiled to a code object with \
...
// THE SAME symbol reader `avarok_core::registry` guards kernel lookups with,
// included by path rather than copied: a build warning that disagreed with the
// runtime refusal would be worse than no warning at all. avarok-core is not a
...
// shared as a file. `crates/avarok-core/src/elf_symbols.rs` documents the format
// subset and why it exists.
#[path = "../avarok-core/src/elf_symbols.rs"]
mod elf_symbols;
use build_parse::{
