...
// Snapshot original: .research/exllamav3_ref/exl3_gemv_kernel.cuh.
// Adaptations (kernel body, per-lane extraction constants and FOLD cadence
// verbatim — they are load-bearing numerical contracts):
//   * the `__global__` template kernel became `inline __device__ void
//     exl3_gemv_kernel_body` so extern "C" __global__ wrappers in
//     exl3_matmul.cu can be selected by name; the
//     __launch_bounds__(CFG == 0 ? 512 : 256) moves to those wrappers
//   * include paths made local (exl3_compat.cuh for CEIL_DIVIDE)
