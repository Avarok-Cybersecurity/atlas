...
/// `AVAROK_AGENTIC_NO_WARM=1` — see the note at the `cargo_target_dir`
/// assignment for what it costs and why it exists.
fn no_warm_cache() -> bool {
    static V: std::sync::OnceLock<bool> = std::sync::OnceLock::new();
    *V.get_or_init(|| std::env::var("AVAROK_AGENTIC_NO_WARM").as_deref() == Ok("1"))
}
...
impl AgenticWebserver {
    ...
    async fn run_iteration(&mut self, index: usize) -> Result<IterationRow> {
        ...
        // (avarok-warm-target)", verbatim from two trajectories. That is the
...
impl Plugin for AgenticWebserver {
    ...
    fn load(&mut self, handle: PluginHandle) -> impl Future<Output = Result<()>> + Send {
        ...
        async move {
            ...
            // `AVAROK_AGENTIC_NO_WARM=1`: no shared target dir at all. The
            ...
            // issued. Nothing in Avarok sets `CARGO_NET_OFFLINE` — warm.rs
            ...
            self.cargo_target_dir = if no_warm_cache() {
                tracing::warn!(
                    "AVAROK_AGENTIC_NO_WARM=1: no shared cargo target dir —                      each sandbox builds cold in its own ./target"
                );
                None
            } else {
