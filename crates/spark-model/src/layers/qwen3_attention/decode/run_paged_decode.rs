...
impl Qwen3AttentionLayer {
    pub(in super::super) fn run_paged_decode(
        ...
        match self.kv_dtype {
            // V4-Flash (DeepSeek-V4) uses MLA with a compressed KV cache (576 dims:
            // 512 latent + 64 rope). Detected via rope > 0; routed to the dedicated
            // MLA paged-decode kernels before the standard per-dtype arms.
            KvCacheDtype::Nvfp4 if is_v4_flash => {
                let kv_cache_dim =
                    (self.mla.as_ref().unwrap().kv_lora_rank + self.mla.as_ref().unwrap().rope)
                        as u32; // 512 + 64 = 576
