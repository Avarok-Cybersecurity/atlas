    ...
    fn empty_recent_returns_none() {
        ...
        assert_eq!(detect(&[sig_text_only("hello world how are you doing today")]), LoopState::None);
    }
    ...
    fn three_identical_intros_fire_loop() {
        ...
        let v = detect(&recent);
        assert!(matches!(v, LoopState::Suppress { .. } | LoopState::Hint { .. }), "got {v:?}");
    }
    ...
    fn moderate_similarity_3_turns_now_suppress() {
        // Regression for the threshold recalibration (2026-04-25).
        ...
        //
        // Pre-recal (HIGH=0.80, MODERATE=0.55): returned `Hint` —
        // model kept looping.
        // Post-recal (HIGH=0.65, MODERATE=0.50): `Suppress` fires.
        ...
        assert!(
            ...
            "post-recal: 4 turns sharing the same long prose stem must Suppress: {v:?}"
        );
    }
    ...
    fn moderate_band_with_065_max_suppress() {
        ...
        assert!(
            matches!(v, LoopState::Suppress { .. }),
            "highly-similar 3-turn run must Suppress post-recal: {v:?}"
        );
    }
    ...
    fn empty_signatures_are_skipped_not_counted() {
        ...
        let intro = "I will create a proper echo server for you. Let me write a clean implementation now.";
        let recent = vec![
            sig_text_only(intro),
