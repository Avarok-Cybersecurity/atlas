...
pub fn process_seq_logits(
    ...
    adaptive_sampling: bool,
            ...
            if a.inside_thinking && !a.force_end_thinking && a.thinking_tokens >= 400 {
                ...
                if sum_exp > 0.0 && 1.0 / sum_exp >= 0.95 {
                    ...
                    if a.consecutive_confident >= 30 {
                        ...
                        tracing::info!("Confidence early stop: top-1 prob >= 0.95 for 30 tokens (after {} thinking tokens)", a.thinking_tokens);
                    }
                } else {
            ...
            if a.inside_thinking && a.force_end_thinking
            ...
            let sampled = sample_with_params_history(f32_bytes, &SamplingParams {
                ...
                lz_penalty: if a.grammar_state.is_some() { 0.0 } else { a.lz_penalty },
                edt_strength: 0.0,
                edt_floor: 0.1,
                ...
                && a.output_tokens.len() >= ENTROPY_COLLAPSE_WARMUP_TOKENS
            {
                ...
                if h < ENTROPY_COLLAPSE_THRESHOLD_NATS {
                    a.entropy_collapse_streak =
                        a.entropy_collapse_streak.saturating_add(1);
                ...
                && a.output_tokens.len() >= ENTROPY_COLLAPSE_WARMUP_TOKENS
            {
                ...
                if dup && token_diversified {
                    a.f27_attractor_streak =
                        a.f27_attractor_streak.saturating_add(1);
                } else if !dup {
            ...
            let logprobs = a.top_logprobs.map(|k| extract_logprobs_from_f32(&f32_logits, sampled, k as usize));
            (sampled, logprobs)
}
