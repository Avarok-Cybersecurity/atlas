...
def main() -> int:
    ...
    out: list[str] = []
    out.append("# Avarok vs HF Reference: Per-Operation Drift Table (Qwen3.6-35B-A3B-FP8)")
    out.append("")
    out.append("## Test setup")
    out.append("- Prompt: canonical 10382-token chat probe (last 5 prompt tokens = [248045, 74455, 198, 248068, 198])")
    out.append("- Avarok image: `avarok-gb10:op-drift` (built today from `avarok-gb10:fp8-much-better` lineage, commit 8d2cc87, native FP8 SSM dispatch)")
    out.append("- HF reference: `Qwen/Qwen3.6-35B-A3B` (BF16, original unquantized weights — the absolute reference)")
    out.append("- Compute device: dgx2 GPU (Avarok) + dgx1 CPU (HF reference forward)")
    ...
    if ok_reliable:
        ...
        if layer_cos:
            out.append(
                ...
                "this is the Avarok-vs-HF[BF16-unquant] residual-stream drift = 'B' comparison."
            )
        out.append("")
    ...
    out.append("   Avarok's Q GEMM is effectively at the FP8 compute ceiling — the FP8")
    out.append("   weight quantization itself is by far the dominant error, not any Avarok-side")
    out.append("   compute inaccuracy.")
    out.append("")
    out.append("2. **MoE block is the dominant Avarok-side drift source.**")
    ...
    out.append("   (which renormalises Avarok-side scale error) while V and K go directly into")
    out.append("   attention without rescaling. Avarok's QK-norm is correct.")
    out.append("")
    ...
    out.append("")
    out.append("6. **No L39 cliff**: Avarok's per-layer hidden_out at L39 = 0.98790 (mid-pack);")
    ...
    out.append("")
    out.append("### Top 20 worst MEANINGFUL ops (Avarok compute headroom)")
    out.append("")
    ...
    out.append("")
    out.append("- **Layer-indexing convention**: Avarok full-attention dumps use ATTN-RELATIVE index (0..9). The cosine script translates Avarok-L0→abs-L3, ..., Avarok-L9→abs-L39 for joining with HF.")
    out.append("- **q_proj_full layout**: Avarok dumps Q+Gate interleaved (`[q_dim*2]`), HF dumps the same shape but unconcatenated. Full-vector cosine still correlates with overall projection quality.")
    out.append("- **ssm.pre_norm**: pre-existing Avarok dump bug — when SSM is configured with FP32 residual, `maybe_dump_gdn_buf` reads N×2 bytes (BF16 semantic) starting at an FP32-byte-stride offset. The resulting blob is half the values, mis-typed. Skip these rows.")
    out.append("- **ssm.conv**: HF Conv1d output is `[1, channels, T+3]`; naive `extract_last` picks the wrong slice (last channel rather than last time-step). Avarok dumps `[8192]` last-token. Shape-mismatch flagged in table.")
    out.append("- **ssm.gnorm**: HF Qwen3_5MoeRMSNormGated reshapes to `[-1, 128]` then norms — the captured hook returns the last (token, head) pair = `[128]`. Avarok dumps the full `[4096]` = 32 heads × 128. Per-head ordering between Avarok and HF differs (Avarok heads-contiguous, HF token×head interleaved), so naive prefix-cosine is not meaningful.")
    out.append("- **ssm.in_proj_qkvz / in_proj_ba**: Qwen3.6 HF splits these into `in_proj_qkv` + `in_proj_z` and `in_proj_a` + `in_proj_b` (4 separate Linear modules); Avarok combines them into a single fused GEMM (`in_proj_qkvz`). No direct 1:1 HF reference.")
    out.append("- **HF gating side-effects**: For SSM layers, `linear_attn.norm(x, z)` applies `z`-gating inside the norm; HF hook captures the gated output. Avarok's gnorm hook is at the same position.")
    ...
    out.append("")
    out.append("```bash")
    out.append("# Avarok op-drift image (commit 8d2cc87 lineage + AVAROK_OP_DUMP hooks)")
