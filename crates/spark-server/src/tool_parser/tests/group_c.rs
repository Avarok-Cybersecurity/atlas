...
use super::super::*;
    ...
    fn parse_mistral_nested_json_args() {
        let input = r#"[TOOL_CALLS]search[ARGS]{"filters":{"year":2025,"tags":["rust","cuda"]},"limit":10}"#;
    ...
    fn streaming_detector_mistral_single() {
        ...
        assert!(
            matches!(&out[1], DetectorOutput::ToolCallStart { name, .. } if name == "get_weather")
        );
    ...
    fn streaming_detector_mistral_split_tokens() {
        ...
        let has_start = outs.iter().any(
            |o| matches!(o, DetectorOutput::ToolCallStart { name, .. } if name == "get_weather"),
        );
        assert!(has_start, "Missing ToolCallStart");
        let has_delta = outs.iter().any(
            |o| matches!(o, DetectorOutput::ToolCallDelta { args, .. } if args.contains("Paris")),
        );
    ...
    fn salvage_rejects_non_identifier_name() {
        ...
        let pipeline = ToolCallPipeline::bare_function_default();
        let (_content, calls) = pipeline.run(input);
        assert!(calls.is_empty(), "non-identifier first param must not salvage");
    }
