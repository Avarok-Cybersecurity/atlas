...
//!
//! ONE LEVER FOR THE FAMILY. The tensor-core state spine's lever selects
//! these two as well wherever the image carries them, because the three
//! kernels are one pipeline and an operator who wants the TC prefill wants all
//! of it. That lever is the COMPILED TARGET's `[defaults] gdn_prefill_tc` with
//! `ATLAS_GDN_PREFILL_TC` overriding — resolved ONCE by `gdn_prefill_fla` and
//! handed here as `requested`, never read a second time from the environment.
//! A second read would be a second answer: `ATLAS_GDN_PREFILL_TC=0` is an
//! explicit OFF under the 2026-09-11 grammar (`layers::ops::target_defaults`),
//! and a presence check here would have turned the twins ON for it while the
//! spine stayed off — the half-enabled prefill that is neither leg of an A/B.
//!
//! The A/B that needs them apart gets `ATLAS_NO_GDN_PREFILL_TC_REMNANTS=1`,
//! which keeps the spine and leaves `wu`/`fwd_o` on their parents — one
//! variable, in the direction that is safe to be wrong about.
...
///
/// `requested` is the family lever ALREADY RESOLVED by the caller —
/// `target_defaults::resolved().gdn_prefill_tc.value`, the same bit and the
/// same read that selects the tensor-core state spine. A parameter rather than
/// a lookup: the spine and the twins must not be able to disagree about what
/// the operator asked for, and only the caller is in a position to read it
/// once for all three.
#[allow(clippy::too_many_arguments)]
