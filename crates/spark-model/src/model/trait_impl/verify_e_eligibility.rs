...
/// admitted only under `AVAROK_HC_BATCH_VERIFY=1`, because their cross-sequence
...
/// `AVAROK_HC_BATCH_VERIFY=1` — opt in to the cross-sequence highway verify.
///
...
pub(super) fn hc_batch_verify_enabled() -> bool {
    ...
    *ON.get_or_init(|| std::env::var("AVAROK_HC_BATCH_VERIFY").as_deref() == Ok("1"))
}

/// `AVAROK_HC_ATTN_FFN_BATCHED` (default on; `=0` restores the per-sequence
...
pub(super) fn hc_attn_ffn_batched() -> bool {
    static ON: std::sync::OnceLock<bool> = std::sync::OnceLock::new();
    *ON.get_or_init(|| std::env::var("AVAROK_HC_ATTN_FFN_BATCHED").as_deref() != Ok("0"))
}

/// `AVAROK_HC_ATTN_CORE_BATCHED` (default on; `=0` restores the per-sequence
...
pub(super) fn hc_attn_core_batched() -> bool {
    static ON: std::sync::OnceLock<bool> = std::sync::OnceLock::new();
    *ON.get_or_init(|| std::env::var("AVAROK_HC_ATTN_CORE_BATCHED").as_deref() != Ok("0"))
}
...
mod tests {
    ...
    /// suite does not set `AVAROK_HC_BATCH_VERIFY`.
    #[test]
