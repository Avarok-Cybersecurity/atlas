...
const DGX2 = node('c', { name: 'dgx2', via: node('b').id });
const DGX3 = node('d', { name: 'dgx3', via: node('b').id });
...
test('a machine vouched for by something absent is still drawn', () => {
  ...
  const orphan = node('e', { name: 'ghost', via: 'f'.repeat(64) });
...
test('a node cannot be vouched for by the local agent', () => {
  ...
  const odd = node('g', { name: 'odd', via: LOCAL.id });
