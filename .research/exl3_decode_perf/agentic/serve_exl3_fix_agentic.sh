#!/bin/bash
# Fixed binary, the PR #834 agentic profile: two MTP drafts, one sequence, PREFIX CACHING ON (user rule: always), ssm-cache-slots 64 as in the TUI launcher.
set -u
PORT=${1:-8890}
BIN=${SPARK_BIN:-/home/ms/.claude/jobs/5a7bd33d/tmp/exl3bench/spark-sharedgemv}
# CHAT_KWARGS overrides the default chat-template kwargs (thinking off = '{"enable_thinking":false}').
KWARGS=${CHAT_KWARGS:-'{"reasoning_effort":"low"}'}
[ "${NOTHINK:-0}" = 1 ] && KWARGS='{"enable_thinking":false}'
cd /home/ms/atlas/.claude/worktrees/exl3-research
export ATLAS_EXL3_NATIVE=1 ATLAS_EXL3_NATIVE_MOE=1 ATLAS_EXL3_NATIVE_DENSE=1
export ATLAS_PLE_MAX_TOKENS=9216 ATLAS_PLE_CACHE_SLOTS=4194304 ATLAS_QSA_MAX_TOKENS=32768
export ATLAS_INTHINK_TOOL_LEAK_OPENERS=0 ATLAS_NO_HW_PRECHECK=1
export ATLAS_QWEN4EXP_MTP=1 ATLAS_QWEN4EXP_MTP_VERIFY=1 ATLAS_DFLASH_SPEC_THINK=1
export ATLAS_QWEN4EXP_MTP_HC_BATCHED=1
export ATLAS_VERIFY_EXL3_ROW_ROUTER=1 ATLAS_VERIFY_EXL3_STABLE_GRID=1
# NO_ROW_ROUTER=1: batched verify router (batch2/batch3 GEMV + batched top-k) instead of per-row.
[ "${NO_ROW_ROUTER:-0}" = 1 ] && unset ATLAS_VERIFY_EXL3_ROW_ROUTER
# BARE=1: rely on the binary's defaults for the router/stable-grid flags (no env at all).
[ "${BARE:-0}" = 1 ] && unset ATLAS_VERIFY_EXL3_ROW_ROUTER ATLAS_VERIFY_EXL3_STABLE_GRID
export ATLAS_NO_VERIFY_ROW_FFN=1
export ATLAS_NO_THINKENDED_GPU_ARGMAX=1 ATLAS_MTP_ACCEPT_DEBUG=1
export RUST_LOG=${RUST_LOG:-info}
export LD_LIBRARY_PATH=/home/ms/nccl/build/lib:/usr/local/cuda/lib64:${LD_LIBRARY_PATH:-}
exec "$BIN" serve \
  --model-from-path /tank/exl3-ckpt/qwen38-flash-next-4.05bpw \
  --model-name qwen3.8-flash-next --kernel-target qwen3.8-flash-next \
  --world-size 1 --bind 127.0.0.1 --port $PORT \
  --max-seq-len 32768 --max-num-seqs 1 --max-batch-size 1 \
  --gpu-memory-utilization 0.72 --kv-cache-dtype bf16 --ssm-cache-slots 64 \
  --request-timeout 1800 --fast-load-prefetch-shards \
  --speculative --num-drafts 2 --enable-prefix-caching \
  --default-chat-template-kwargs "$KWARGS"
