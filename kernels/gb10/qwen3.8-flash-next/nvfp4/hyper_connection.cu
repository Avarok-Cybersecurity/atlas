...
// ───────────────────── Decode-rows VARIANTS under measurement (2026-09-09) ────
// provenance-id: 526f6e616c6420522e205374657369616b
// Selected by ATLAS_HC_UP_KERNEL / ATLAS_HC_DOWN_KERNEL (dispatcher table in
// hyper_connection_lowrank.rs); the DRAM-true microbench (hc_rows_microbench,
// 32 weight copies cycled) is the yardstick. Same math per element as the
// kernels above; the reduction order per row is fixed per variant, so every
// variant passes the T=3-vs-T=1 byte test on its own.

// hc_dec_up_v3: eight lanes per row, four rows per warp, FOUR warps per block
// (one warp per stream, 4 hidden columns per block), `low` read straight from
// global (L1/L2-hot, no smem fill + barrier ahead of the weight loads).
// Grid: H/4.  Block: hc * 32.  smem: hc*4*T floats (dynamic).
extern "C" __global__ void hc_dec_up_v3(
    ...
    extern __shared__ float qhc_v3_smem[];   // [hc*4, QHC_DEC_MAX_T]
    const unsigned int tid = threadIdx.x;
    const unsigned int lane = tid & 31u;
    const unsigned int s = tid >> 5;                 // stream = warp
    const unsigned int dl = lane >> 3;               // 0..3
    const unsigned int c = lane & 7u;
    const unsigned int H = hidden_size;
    const unsigned int hc_dim = hc * H;
    const unsigned int d = blockIdx.x * 4u + dl;
    const unsigned int i = s * H + d;
    const unsigned int slice = rank / 8u;
    const unsigned int chunks = slice >> 3;
    const __nv_bfloat16* urow = up_w + (size_t)i * rank + (size_t)c * slice;
    uint4 raw[QHC_UP_MAX_CHUNKS];
    #pragma unroll
    ...
    for (int u = 0; u < QHC_UP_MAX_CHUNKS; ++u) {
        if (u < (int)chunks) {
            ...
            for (int t = 0; t < QHC_DEC_MAX_T; ++t) {
                if (t < (int)num_tokens) {
                    const float* lo = low + (size_t)t * rank + r0 + 8u * u;
                    const float4 a = *reinterpret_cast<const float4*>(lo);
                    const float4 b = *reinterpret_cast<const float4*>(lo + 4);
                    acc[t] += w[0] * a.x + w[1] * a.y + w[2] * a.z + w[3] * a.w
                            + w[4] * b.x + w[5] * b.y + w[6] * b.z + w[7] * b.w;
                }
            }
    ...
    if (c == 0) {
        ...
        for (int t = 0; t < QHC_DEC_MAX_T; ++t) {
            if (t < (int)num_tokens) {
                qhc_v3_smem[((size_t)s * 4u + dl) * QHC_DEC_MAX_T + t] =
                    qhc_sigmoid(acc[t]) * normed[(size_t)t * hc_dim + i];
            }
        }
    }
    __syncthreads();
    if (tid < 4u) {
        const unsigned int dd = blockIdx.x * 4u + tid;
        const float inv_hc = 1.0f / (float)hc;
        #pragma unroll
        for (int t = 0; t < QHC_DEC_MAX_T; ++t) {
            if (t < (int)num_tokens) {
                float mixed = 0.0f;
                for (unsigned int s2 = 0; s2 < hc; ++s2) {
                    mixed += qhc_v3_smem[((size_t)s2 * 4u + tid) * QHC_DEC_MAX_T + t];
                }
                y_out[(size_t)t * H + dd] = __float2bfloat16(mixed * inv_hc);
            }
        }
...
// hc_dec_down_v4: EIGHT warps per weight row (one row per 256-thread block,
// each warp a contiguous hc_dim/8 slice, every load of the slice in flight),
// partials summed in warp order through shared memory. Grid: rows.
#define QHC_DOWN4_SPLIT 8
#define QHC_DOWN4_UNROLL 8
extern "C" __global__ void hc_dec_down_v4(
    const float* __restrict__ normed,
    const __nv_bfloat16* __restrict__ down_w,
    const __nv_bfloat16* __restrict__ inject_w,
    ...
    __shared__ float smem_part[QHC_DOWN4_SPLIT][QHC_DEC_MAX_T];
    const unsigned int lane = threadIdx.x & 31u;
    const unsigned int part = threadIdx.x >> 5;
    ...
    const __nv_bfloat16* wrow = is_inj
        ? inject_w + (size_t)(row - rank) * hc_dim
        : down_w + (size_t)row * hc_dim;
    const unsigned int seg = hc_dim / QHC_DOWN4_SPLIT;
    const unsigned int base = part * seg;
    const unsigned int end = base + seg;
    float acc[QHC_DEC_MAX_T];
    #pragma unroll
    for (int t = 0; t < QHC_DEC_MAX_T; ++t) acc[t] = 0.0f;
    for (unsigned int i0 = base + lane * 8u; i0 < end; i0 += 256u * QHC_DOWN4_UNROLL) {
        uint4 raw[QHC_DOWN4_UNROLL];
        #pragma unroll
        for (int u = 0; u < QHC_DOWN4_UNROLL; ++u) {
            const unsigned int i = i0 + 256u * u;
            raw[u] = (i < end) ? *reinterpret_cast<const uint4*>(wrow + i) : make_uint4(0u, 0u, 0u, 0u);
        }
        #pragma unroll
        for (int u = 0; u < QHC_DOWN4_UNROLL; ++u) {
            const unsigned int i = i0 + 256u * u;
    ...
    if (part == 0 && lane == 0) {
        ...
        for (int t = 0; t < QHC_DEC_MAX_T; ++t) {
            if (t < (int)num_tokens) {
                float v = 0.0f;
                #pragma unroll
                for (int q = 0; q < QHC_DOWN4_SPLIT; ++q) v += smem_part[q][t];
