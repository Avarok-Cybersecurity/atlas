...
/// The serve log line, in a child module so the formatter and the table it
/// reports sit in separate files at the 500-line ceiling. Re-exported, so
/// `target_defaults::summary_line` and `::format_levers` still resolve.
#[path = "target_defaults_line.rs"]
mod line;
pub use line::{format_levers, summary_line};
