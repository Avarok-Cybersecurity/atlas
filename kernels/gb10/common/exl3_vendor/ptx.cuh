...
//
// Vendored VERBATIM from turboderp's ExLlamaV3 (https://github.com/turboderp-org/exllamav3)
// Copyright (c) 2025 turboderp — MIT license.
// Snapshot original: .research/exllamav3_ref/ptx.cuh (only this header block added).
// NOTE upstream comment on cp_async_pred: it produces incorrect code when
// compiling for Blackwell — the kernels use `if (pred) cp_async(...)` instead.
